...
> **注意**：将 `docker-compose-synology.yml` 手动重命名为 `docker-compose.yml` 后使用

## 镜像说明

| 镜像 | 用途 | 构建频率 |
|------|------|----------|
| `ghcr.io/lichncjx/supervision-system:latest` | Next.js 应用 | 每次 main 分支更新 |
| `ghcr.io/lichncjx/supervision-system-migrate:latest` | 数据库迁移 | 仅 prisma/package.json/Dockerfile 等基础设施文件变化时 |

### migrate 镜像按需构建

- **普通业务代码变化**：只重建 app 镜像，不重建 migrate 镜像
...
deploy.sh 仍会执行 migrate，但如果数据库结构没有变化，迁移会快速结束。
