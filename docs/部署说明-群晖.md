...
部分 migration 新增字段后，历史数据需要回填。回填 SQL 统一放在 `docs/` 目录下。

**当前需要回填的 migration**：
- Phase 2：`deptLeaderId` / `deptLeaderName` / `deptManagerId` / `deptManagerName` 四列，需通过回填 SQL 将旧字段值匹配到新字段。
...
docker-compose exec postgres psql -U postgres -d supervision -f /path/to/docs/backfill-*.sql
