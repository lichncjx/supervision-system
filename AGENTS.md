...
4. 审批记录不得被单独删除；创建人或系统管理员删除整个 `DRAFT` 事项时的级联删除例外以 `docs/core/业务规则.md` 为准。
5. 统计口径必须和列表查询口径一致。
6. 首页卡片数量必须能和点击后的列表结果对应。
7. 待我处理、待审批、待办理等统计和列表口径以 `docs/core/业务规则.md` 为准，不在本文件重复维护细分规则。
8. 涉及权限、状态、审批、统计的修改必须说明影响范围。
