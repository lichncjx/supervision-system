...
async function createWork(cookies, data) {
  const isStructuredWork = data.type === 'priority' || data.type === 'main'
  const res = await request(
    'POST',
    '/api/works',
    {
      assessmentYear: data.assessmentYear ?? 2026,
      ...(isStructuredWork ? { workNode: data.workNode ?? '回归测试工作节点' } : {}),
      ...data,
    },
    cookies,
  )
...
async function main() {
  ...
  try {
    ...
    assert(
      'todo proposal stores beforeApprovalStatus',
      todoState.body?.beforeApprovalStatus === 'DRAFT',
    )
    ...
    assert(
      'company approver is selected',
      todoState.body?.currentApproverId === vicePresident.user.id,
    )
    ...
    assert(
      'proposal helper fields are cleared',
      !todoState.body?.beforeApprovalStatus && !todoState.body?.approvalType,
    )
    ...
    assert(
      'department manager creates priority draft',
      Boolean(returned.body?.id),
      `id=${returned.body?.id}`,
    )
    ...
    assert(
      'returned draft keeps reject trace',
      Boolean(returnedState.body?.rejectReason || returnedState.body?.rejectedFromStatus),
    )
    ...
    assert(
      'company leader creates todo draft',
      Boolean(companyTodo.body?.id),
      `id=${companyTodo.body?.id}`,
    )
    ...
    assertStatus(
      'company todo submit enters PENDING_DECOMPOSE',
      companyTodoState,
      'PENDING_DECOMPOSE',
    )
    ...
    const decomposeSubmit = await workflow(companyTodo.body.id, deptManager.cookies, 'decompose', {
      nodes: [
        { title: `${RUN_ID} node`, responsiblePerson: 'owner', planCompleteTime: '2026-12-31' },
      ],
      comment: 'decomposed',
    })
    assert(
      'department manager submits decomposition',
      decomposeSubmit.statusCode === 204,
      responseSummary(decomposeSubmit),
    )
    companyTodoState = await workflowWorkResponse(
      decomposeSubmit,
      companyTodo.body.id,
      deptManager.cookies,
    )
    ...
    assert(
      'decompose beforeApprovalStatus is PENDING_DECOMPOSE',
      companyTodoState.body?.beforeApprovalStatus === 'PENDING_DECOMPOSE',
    )
    ...
    assert(
      'department leader rejects decomposition',
      decomposeReject.statusCode === 204,
      responseSummary(decomposeReject),
    )
    companyTodoState = await workflowWorkResponse(
      decomposeReject,
      companyTodo.body.id,
    ...
    assertStatus(
      'decompose reject returns to PENDING_DECOMPOSE',
      companyTodoState,
      'PENDING_DECOMPOSE',
    )
    ...
    assert(
      'department manager creates priority for cancel',
      Boolean(priority.body?.id),
      `id=${priority.body?.id}`,
    )
    ...
    assert(
      'main leader approver role is PRESIDENT',
      priorityState.body?.currentApproverRole === 'PRESIDENT',
    )
    ...
    const legacyStatusFilter = await request(
      'GET',
      '/api/works?status=APPROVED',
      null,
      admin.cookies,
    )
    assert(
      'legacy status filter is rejected',
      legacyStatusFilter.statusCode === 400,
      `statusCode=${legacyStatusFilter.statusCode}`,
    )

    const inProgressFilter = await request(
      'GET',
      '/api/works?status=inProgress',
      null,
      admin.cookies,
    )
    const statuses = Array.isArray(inProgressFilter.body)
      ...
      : []
    assert(
      'inProgress filter only returns IN_PROGRESS',
      inProgressFilter.statusCode === 200 && statuses.every((status) => status === 'IN_PROGRESS'),
      statuses.join(','),
    )
  } finally {
  ...
  console.log()
  console.log(
    `[${RUN_ID}] summary: ${results.length - failed.length} passed, ${failed.length} failed`,
  )
