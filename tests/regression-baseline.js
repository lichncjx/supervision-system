...
async function runComprehensiveTests() {
  ...
  logTest('  待办状态变为 IN_PROGRESS', todoVPAfterApprove.body?.status === 'IN_PROGRESS', `Status: ${todoVPAfterApprove.body?.status}`);
  ...
  logTest('  提交后状态 PENDING_COMPLETE', todoVPAfterEvidence.body?.status === 'PENDING_COMPLETE', `Status: ${todoVPAfterEvidence.body?.status}`);
  ...
  logTest('  待办最终状态 COMPLETED', todoVPFinal.body?.status === 'COMPLETED', `Status: ${todoVPFinal.body?.status}`);
  ...
  logTest('  部门发起待办提交后状态 PENDING_DEPT', todoMgrAfterSubmit.body?.status === 'PENDING_DEPT', `Status: ${todoMgrAfterSubmit.body?.status}`);
  ...
  logTest('  取消前待办状态 IN_PROGRESS', todoCancelBefore.body?.status === 'IN_PROGRESS', `Status: ${todoCancelBefore.body?.status}`);
  ...
  logTest('  取消申请后状态 CANCELLING', todoAfterCancelApply.body?.status === 'CANCELLING', `Status: ${todoAfterCancelApply.body?.status}`);
  ...
  logTest('  待办取消后状态 CANCELLED', todoAfterCancel.body?.status === 'CANCELLED', `Status: ${todoAfterCancel.body?.status}`);
  console.log();
  ...
  logTest('  重点工作已审批状态 APPROVED', priorityAfterApprove.body?.status === 'APPROVED', `Status: ${priorityAfterApprove.body?.status}`);
  ...
  logTest('  部门领导审批后状态 CANCELLING，currentApproverRole=VICE_PRESIDENT',
    ...
    `Status: ${priorityAfterDeptApprove.body?.status}, Role: ${priorityAfterDeptApprove.body?.currentApproverRole}`);
  ...
  logTest('  公司主管领导审批后状态 PENDING_MAIN_LEADER_CANCEL',
    ...
    `Status: ${priorityAfterVP.body?.status}`);
  ...
  logTest('  重点工作取消完整链 CANCELLED', priorityAfterFullCancel.body?.status === 'CANCELLED', `Status: ${priorityAfterFullCancel.body?.status}`);
  console.log();
  ...
  logTest('  主要工作已审批状态 APPROVED', mainAfterApprove.body?.status === 'APPROVED', `Status: ${mainAfterApprove.body?.status}`);
  ...
  logTest('  取消申请后状态 CANCELLING', mainAfterCancelSubmit.body?.status === 'CANCELLING', `Status: ${mainAfterCancelSubmit.body?.status}`);
  ...
  logTest('  部门领导审批后状态 CANCELLING，currentApproverRole=VICE_PRESIDENT',
    ...
    `Status: ${mainAfterDeptApprove.body?.status}, Role: ${mainAfterDeptApprove.body?.currentApproverRole}`);
  ...
  logTest('  主要工作取消完整链 CANCELLED', mainAfterFullCancel.body?.status === 'CANCELLED', `Status: ${mainAfterFullCancel.body?.status}`);
  console.log();
  ...
  logTest('  退回后状态 REJECTED', afterReject1.body?.status === 'REJECTED', `Status: ${afterReject1.body?.status}`);
  ...
  logTest('  重新提交后状态 PENDING_DEPT（创建人是部门主管）', afterResubmit1.body?.status === 'PENDING_DEPT',
    `Status: ${afterResubmit1.body?.status}`);
  ...
  logTest('  公司级退回后状态 REJECTED', afterReject2.body?.status === 'REJECTED', `Status: ${afterReject2.body?.status}`);
  ...
  logTest('  重新提交后状态 PENDING_COMPANY（创建人是部门领导）', afterResubmit2.body?.status === 'PENDING_COMPANY',
    `Status: ${afterResubmit2.body?.status}`);
  console.log();
